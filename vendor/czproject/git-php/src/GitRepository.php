<?php
	...
	class GitRepository implements IGit
	{
		...
		public function logs($filename = '') {
			$cmd = 'git log --date=short --pretty=format:"%h||%an||%ar||%s||%cd" '.$filename;
			return $this->extractFromCommand($cmd, 'trim');
		}

		public function status($subdir = '') {
			$this->begin();
			$out = $this->extractFromCommand('git status '.$subdir, 'trim');
			$this->end();

			return $out;
		}

		public function changedFiles ($subdir = '') {
			$arr = $this->status($subdir);
			
			$files = array();
			foreach($arr as $line) {
				if (strpos($line, "new file:") !== false) {
					$file_line = explode("new file:", $line);
					$files[] = array('file' => trim($file_line[1]), 'status' => 'new');
				} else if (strpos($line, "modified:") !== false) {
					$file_line = explode("modified:", $line);
					$files[] = array('file' => trim($file_line[1]), 'status' => 'modified');
				}
		...
		public function changedFilesByFileName ($subdir = '') {
			$arr = $this->status($subdir);
		...
		public function setConfig($username, $email = '') {
			$this->begin();
			$out = $this->extractFromCommand('git config user.name "'.$username.'"', 'trim');
			$this->end();

			if ($email) {
				$this->begin();
				$out = $this->extractFromCommand('git config user.email "'.$email.'"', 'trim');
				$this->end();
			}
		...
		public function setConfigExt ($command) {
			$this->begin();
			$out = $this->extractFromCommand('git config --local '.$command, 'trim');
			$this->end();
