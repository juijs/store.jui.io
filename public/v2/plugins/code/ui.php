...
jui.defineUI("ui.filetree", [], function () {
	var FileTree = function() {
			...
			var $el;
			var $contextMenu;
			var newFolderWin, newFileWin, moveToWin;
			...
			this.initWindow = function () {
				...
				$([newFolderWin.root, newFileWin.root, moveToWin.root]).on('click', '.close', function () {
					$(this).closest('.window')[0].jui.hide();
				});

				$([newFolderWin.root, newFileWin.root, moveToWin.root]).on('click', '.apply', function () {
					var win = $(this).closest('.window')[0].jui;
					win.emit('apply');
				});

				$([newFolderWin.root, newFileWin.root]).on('keyup', 'input[type=text]', function (e) {
					if (e.keyCode == 13) {
						var win = $(this).closest('.window')[0].jui;
						win.emit('apply');
					}
	...
	<div class="content">
		<div class="commit-container">
			...
			<div class="commit-message">
				...
				<div style='margin-top:5px;'><textarea placeholder="Update Code"></textarea> </div>
			</div>
